...
public class ExampleController {
	...
	public String ex1(HttpServletRequest req, Model model) {
		...
		// 단일값(숫자, 문자열) Model을 이용해서 html로 전달
		model.addAttribute("productName", "종이컵");
		model.addAttribute("price", 2000);
		
		// 복수값(배열, List) Model 이용해서 html 로 전달
		...
		stdList.add(new Student("22222", "김이번", 23)); 		
	...
	public String ex2(Model model) {
		model.addAttribute("str", "<h1>테스트중 &times</h4>");
	...
	public String pathVariableTest(@PathVariable("path") int path) {
		// controller 에서 해야 하는일 동일한 경우에
		// example/ex3/1, example/ex3/2, example/ex3/3...
		// 주소 중 {path} 부분의 값을 가져와서 매개 변수로 저장
