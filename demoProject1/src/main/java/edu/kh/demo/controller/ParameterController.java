...
public class ParameterController {
	...
	public String paramTest2(@RequestParam("title") String title, @RequestParam("writer") String writer, 
			@RequestParam("price") int price, // 형변환이 필요한 경우에는 파라미터 value 값 필수 작성되어 있어야 함
	...
	// @RequestParma Map<String,object> parmaMap
	// -> 제출된 모든 파라미터가 Map에 저장된다
	// -> 같은 name 속성을 가진 파라미터는 String[], List 로 저장 X
	// 첫번째로 제출된 value 값이 저장됨
	log.debug("paramMap: " + paramMap);
	// {color=Red, fruit=Apple, productName=김, expirationDate=0121-12-04}
	...
	public String paramTest4(@ModelAttribute MemberDTO inputMember) {
		...
		//(현재 위치가 중요! -> 현재 경로의 가장 마지막 부분을 redirect: 이 주소 로 변경하기 때문에)
		//redirect:/param/main
	}
