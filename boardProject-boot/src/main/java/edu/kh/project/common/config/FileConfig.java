...
public class FileConfig implements WebMvcConfigurer {
	...
	private String profileResourceHander;
	...
	// 요청 주소에 따라
	// 서버 컴퓨터에 어떤 경로에 접근할 지 설정

	@Override
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		// ResourceHandelerRegistry :
		// Spring MVC 에서 정적 리소스 의 요청을 처리하기 위해 사용하는 클래스
		...
		// -> 클라이언트가 /myPagw/file/** 패턴으로 이미지를 요청할 때
		// 서버 폴더 경로중 C:/uploadFiles/temp/ 로 연결하겠다(여기서 이미지 찾겠다)
		registry
	...
	// MultipartResolver 객체를 생성하여 Bean 으로 등록
	// -> Bean 으로 등록하면서 위에서 만든 MulitpartConfigElement 자동으로 이용함
	@Bean
	public MultipartResolver multipartResolver() {
		// MultipartResolver : MultipartFile 을 처리해주는 해결사
		// MultipartResolver 는 클라이언트로부터 받은 Multipart 요청을 처리하고
