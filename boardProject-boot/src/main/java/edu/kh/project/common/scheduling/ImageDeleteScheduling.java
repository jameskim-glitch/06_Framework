...
public class ImageDeleteScheduling {
	...
	//@Scheduled(cron = "0,30 * * * * *")
	@Scheduled(cron = "0 0 0 1 * *") // 매달 1일 마다 수행
	public void scheduling() {
		...
		if(!serverImageList.isEmpty()) {// 서버에 이미지가 있을 경우
			for(File serverImage : serverImageList) {
				// dbImageList 에 존재하지 않는 파일명인지 확인
				
				// File.getName() : 서버 파일 이름
				
				// List.indexOf(객체) : 
				// List 에 객체가 존재하면 존재하는 index 번호 반환
				// 존재하지 않으면 -1 반환하는 메서드
...
*
*
*  @Scheduled(cron = "30 * * * * *") // 시계초 단위가 30초 일때 수행
*  @Scheduled(cron = "0,30, * * * * *") // 시계 초 단위가 0,30인 경우 수행
*  @Scheduled(cron = "0 0 12 * * *") // 12시 정각에 수행(24시 기준이므로 정오에만)
