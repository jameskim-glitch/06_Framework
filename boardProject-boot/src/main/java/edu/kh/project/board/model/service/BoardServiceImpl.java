...
//		log.debug("boardList 결과 : {}", boardList);
	...
	public Board selectOne(Map<String, Integer> map) {
		...
		// 2. 수행하려는 SQL이
		// 1) 모두 SELECT 이면서
		// 20 먼저 조회된 결과 중 일부를 이용해서
		// 나중에 주생회는 SQL 의 조건으로 삼을수 있는 경우
		// -> Mybatis 의 <resultMap>, <collection> 태그를 이용해서
		// mapper 메서드 1회 호출 로 여러 SELECT 한 번에 수행 가능
	...
	public int boardLike(Map<String, Integer> map) {
		...
		// 1. 좋아요가 체크된 경우 (likeCheck == 1)
		// -> BOARD_LIKE 테이블에 DELETE 수행
		if(map.get("likeCheck") == 1) {
			result = mapper.deleteBoardLike(map);
		} else {
			// 2. 좋아요가 해제된 경우 (likeCheck == 0)
	...
	public Map<String, Object> searchList(Map<String, Object> paramMap, int cp) {
		...
		// 4. 목록 조회 + Pagination 객체를 Map 으로 묶음
		Map<String, Object> map = new HashMap<>();
