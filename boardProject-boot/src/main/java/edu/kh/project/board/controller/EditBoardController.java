...
public class EditBoardController {
	...
	/** 게시글 작성
	 * @param boardCode : 어떤 게시판에 작성될 글인데 구분
	 * @param inputBoard : 입력된 값( 제목, 내용...) 세팅되어 있음(커맨드 객체)
	 * @param loginMember : 로그인한 회원 번호를 얻어오는 용도
	 * @param images : 제출된 file 타입 input 태그가 전달한 데이터들(이미지 파일..._)
	 * @param ra
	 * @return
	 */
	...
	public String boardInsert(@PathVariable("boardCode") int boardCode,
		 ...
		 *  - 5개 모두 업로드 O -> List 안에 0~4 번 인덱스에 파일 저장됨
		 ...
		 * (제출은 되어 있는데 데이터는 없음)
		 * 
		 * -> 파일 선택이 안된 input 태그값을 서버에 저장하려고 하면 오류 발생함!
		 * 
		 * [해결방법]
		 * - 무작정 서버에 저장 X
		 * -> List 의 각 인덱스에 들어있는 MultipartFile 에 실제로
		...
		// -> inputBoard 에는 총 네가지 세팅됨 (boardTitle, boardContent, boardCode, memberNo)
		
		// 2. 서비스 메서드 호출 후 결과 반환받기
		// -> 성공시 [상세 조회] 를 요청할 수 있도록
		// 삽입된 게시글의 번호를 반환받기
	...
	public String boardUpdate(@PathVariable("boardCode") int boardCode, 
		...
		if(board == null) {
			...
			// 해당 글을 상세 조회 리다이렉트(/board/1/2000)
	 ...
	 * @param images : 제출된 input type = "file" 모든 요소 (이미지파일)
	 * @param ra
	...
	public String boardUpdate(@PathVariable("boardCode") int boardCode, 
		...
		if(result > 0) {
			...
			message = "수정 실패";
			// 현재 : /editBoard/1/3998/update?cp=1 (POST)
			path = "update"; // 목표 : /editBoard/1/3998/update?cp=1 (GET)
	...
	@RequestMapping(value = "{boardCode:[0-9]+}/{boardNo:[0-9]+}/delete", method = {RequestMethod.GET,RequestMethod.POST}) // 메소드를 안쓰면 모든 요청을 받을 수 있다
	public String boardDelte(@PathVariable("boardCode") int boardCode,
							 @PathVariable("boardNo") int boardNo,
