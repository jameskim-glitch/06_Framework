...
public class AdminController {
	...
	public ResponseEntity<List<Member>> newMember() {
		try {
			List<Member> memberList = service.newMember(); // ✅ 리스트로 받기
	...
	public ResponseEntity<Object> newMemberCount(){
		try {
			int count = service.newMemberCount();
	...
	public ResponseEntity<Object> selectWithdrawnMemberList(){
		// 성공 시 List<Member> 반환, 실패시 String 반환 -> Object
		try {
			List<Member> withDrawnMemberList = service.withDrawnMemberList();
	...
	public ResponseEntity<Object> selectDelteBoardList(){
		// 성공 시 List<Board> 반환, 실패시 String 반환 -> Object
		try {
			List<Board> delteBoardList = service.selectDelteBoardList();
			return ResponseEntity.status(HttpStatus.OK).body(delteBoardList);
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body("삭제된 게시글 목록조회 목록 조회 중 문제 발생 : " + e.getMessage());
		}
	}
	/** 탈퇴한 회원 복구
	 * @param member
		...
		try {
			...
			if(result>0) {
				...
				return ResponseEntity.status(HttpStatus.BAD_REQUEST)
						.body("유효하지 않은 회원번호입니다" + member.getMemberNo());
			}
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
	...
	public ResponseEntity<String> restoreBoard(@RequestBody Board board){
		try {
			...
			if(result>0) {
				...
				// BAD_REQUEST : 400 -> 요청 구문이 잘못되었거나 유효하지 않음.
				return ResponseEntity.status(HttpStatus.BAD_REQUEST)
						.body("유효하지 않은 게시글 번호입니다" + board.getBoardNo());
			}
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
	...
	public ResponseEntity<List<Member>> AdminAccount() {
		try {
			List<Member> memberList = service.AdminAccount(); // ✅ 리스트로 받기
			return ResponseEntity.status(HttpStatus.OK).body(memberList);
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
		}
