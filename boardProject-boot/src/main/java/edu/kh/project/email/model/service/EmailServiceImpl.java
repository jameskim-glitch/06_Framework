...
public class EmailServiceImpl implements EmailService {
		...
		// SpringTemplateEngine : 타임 리프를 이용해서 html 코드 -> java 코드 변환
		...
		public String sendEmail(String htmlName, String email) {
			...
			try {
				...
				e.printStackTrace();
				return null; // 메일 발송 실패시 null 발송
			}
			
		}
		// HTML 탬플릿에 데이터를 바인딩 하여 최종 HTML 생성 메서드
		private String loadHtml(String authKey, String htmlName) {
			// Context : 타임리프에서 제공하는 HTML 템플릿에 데이터를 전달하기 위해서 사용하는 클래스
			
			Context context = new Context(); // 템플릿에 바인딩할 데이터를 담는 상자
			context.setVariable("authKey", authKey); // 템플릿에서 사용할 변수 authKey 에 값 설정
		...
		private boolean storeAuthKey(Map<String, String> map) {
			...
			// 2. UPDATE 수행 시 0이 반환됨 ==  UPDATE 안됨 == INSERT 된 내용이 없었다
			// 			INSERT 수행하면 된다.
			if(result==0) {
		...
		// 인증 번호 발급 메서드
		//  UUID 를 사용하여 인증키 생성
		// (Univasally Unique IDentify) : 전 세계에서 고유한 식별자를 생성하기 위한 표준 
