...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async e => {
        ...
        // 기존 닉네임에 저장되어 있는 요소의 value 값 얻어오기
        const currentNickname = document.querySelector("#currnetNickname").value;
        // 기존 닉네임과 새로 입력된 닉네임이 다르면 중복 검사 시도하기
        // -> 같으면 변경된 적 없다 -> 중복 검사 진행 안함!
        if(currentNickname !== memberNickname.value){
            // 비동기 요청 (fetch() API 이용)
            // async / await 사용
            // async : 비동기 함수를 만들 때 사용되는 키워드("이 함수 내에는 시간이 걸리는 작업이 있다!")
            // await : 비동기 작업의 결과를 기다릴 때 사용 키워드
            // -> !!! 반드시 async 함수 안에서만 사용가능!
            // -> '이 작업이 끝날 때 까지 기다려 주세요"
...
// 위 의 모든 검등을 통과햇을 때만 폼 제출
updateInfo.submit();
/*
submit 이벤트 핸들러 함수 내에서 fetch를 이용한 비동기 함수를 실행중 일 때
e.preventDefault() 가 비동기 함수 이후에 호출되면 이미 form 이 submit 되어버린 다음일 수 있음
...
// 주소 검색 버튼 클릭 시 다음 주소 API 실행 (버튼이 존재할 때만)
