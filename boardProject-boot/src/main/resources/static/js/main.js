...
// 배열.map(함수) : 배열의 각 요소를 이용해 콜백 함수 수행 수 결과 값으로 새로운 배열을 만들어서 반환하는 JS 내장 함수
...
if(loginEmail != null){ // 로그인 폼의 이메일 input태그가 화면상에 존재할 때
 ...
 // saveId 값이 있을 경우
 if(saveId != undefined){
  loginEmail.value = saveId; // 쿠키에서 얻어온 이메일 값을 input 요소의 value 에 세팅
  // 아이디 저장 체크박스에 체크해두기
