// REST(REpresantational State Transfer) API 

// /comments (GET) 
// /comments (POST)

// 자원(데이터, 파일)을 이름(주소)으로
// 구분하여(REpresantational) 자원의 상태(State)를 주고 받는것(Transfer)
...
const selectCommentList = () => {
  ...
  fetch("/comments?boardNo=" + boardNo) // GET 방식 요청
    ...
    .then(commentList => {
      ...
      // 조회된 commentList 를 이용해서 새로운 댓글 목록 출력
      // ul태그(댓글 목록 감싸는 요소)
...
const showInsertComment = (parentCommentNo, btn) => {
  ...
  // 매개 변수에 +문자열+ 작성 시 Number타입으로 형변환 라하
...
const insertChildComment = (parentCommentNo, btn) => {
  ...
  const data = {
    ...
    "boardNo": boardNo, // 어느 게시글에 작성된 댓글인가?
