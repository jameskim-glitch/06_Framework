...
memberEmail.addEventListener("input", (e) => {
  ...
  // 영어, 숫자, 특수문자(. _ % + -)를 허용
  // 이메일 형식으로 작성
  // 도메인 부분은 최소 2개의 문자로 끝나야 함 (.com , .net, .org, .kr 등)
...
sendAuthKeyBtn.addEventListener("click", ()=> {
  ...
  // 새로 발급받은 인증번호 확인 전까진 checkObj.authKey 는 false
  checkObj.authKey = false;
  //인증번호 발급 관련된 메시지 지우기
  authKeyMessage.innerText="";
  ...
  authKeyMessage.classList.remove("confirm", "error"); // 검정색 글자
  alert("인증번호가 발송되었습니다");

  // setInterval(콜백 함수, 지연시간(ms))
  // - 지연시간(ms) 만큼 시간이 지날 때 마다 콜백 함수 수행
